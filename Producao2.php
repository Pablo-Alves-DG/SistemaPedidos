...
<script src="JS/Menu.js"></script>
            ...
            while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
                ...
                echo "<tr>
                    ...
                    <select class='Status_Unico' id='Status[" . $li . "]'>" . "
                                                          <option value='Ag_Emissao_NF'>Ag. Emissão de NF</option>
    ...
    <div id="tabela2" class="box">
        <table class="tabela2">
            <tbody>
                ...
                <tr>
                    <th>Data da Produção: <input id="data_producao" type="datetime-local" name="item" size="25"></th>
                </tr>
                <tr>
                    <th>Status Todos:</th>
                </tr>
    ...
    <form style="display: none;" id="formtemp" action="PHP/saida_producao.php" method="post">
        <table id="tabtemp">
            <tbody>
                <tr></tr>
            </tbody>
    ...
    <script>
        function enviar() {
            ...
            for (var ref = 0, linha; linha = dados.rows[ref]; ref++) {
                ...
                t9.innerHTML = "<input id='Data_Producao[" + li + "]' type='text' name='Data_Producao[" + li + "]' value='" + document.getElementById('data_producao').value + "'>";
        ...
        function datenow() {
            var data = new Date();
            document.getElementById('data_producao').value = data.getFullYear() + '-' + ('0' + (data.getMonth() + 1)).slice(-2) + '-' + ('0' + data.getDate()).slice(-2) + 'T' + ('0' + data.getHours()).slice(-2) + ':' + ('0' + data.getMinutes()).slice(-2);
        }
    </script>
</body>
